...
## 14. O que a auditoria da 0.31.0 encontrou

Esta secao existe porque a 0.31.0 parecia pronta.  O `conferir.py` dava
zero, o `progresso.py` dava numeros bonitos, e mesmo assim havia texto em
ingles na tela desde a **primeira versao** e 156 falas nossas a um rebuild
de distancia de sumir.  Nenhum dos dois problemas aparecia em qualquer
ferramenta que o projeto tinha.  Leia antes de escrever o proximo lote.

### 14.1 O erro mais caro: `colher.py` nao via `Strings.source(`

O motor tem DUAS formas de marcar uma frase para o registro `strings`:

```lua
Strings("But, it failed!")              -- traduz na hora
Strings.source("Oh!\nA bite!")          -- so marca; traduz no ponto de uso
```

A segunda existe para tabelas montadas no `require`, antes de
`Strings.load` ter catalogo: traduzir ali congelaria o ingles.  A funcao e
identidade (`src/core/Strings.lua:128`) e a busca acontece depois, usando o
proprio ingles como chave.

O `colher.py` casava `\b(?:S|Strings)\(` -- **parentese logo depois de
`Strings`**.  `Strings.source(` tem um ponto no meio e nunca casou.
Resultado: **212 chaves nunca entraram no `strings_en.json`**, 184 delas
desenhadas por codigo `gen2`.  Ficaram em ingles na tela por 31 versoes:
a pesca, todo CUT/SURF/WATERFALL/STRENGTH, o HEADBUTT, a cura do CENTRO
POKéMON, o premio em dinheiro de cada batalha, o relogio do inicio.

Consertado na 0.32.0: `colher.py` ganhou `SOURCE` e `fatias`, que tambem
juntam concatenacao (`Strings.source("a" .. "b")`, inclusive quebrada em
varias linhas).

**A licao geral, que vale para o proximo:** quando uma frase aparece em
ingles no jogo, a pergunta NAO e "falta traduzir?".  E, nesta ordem:

1. A chave existe no `strings_en.json`?  Se nao, o colhedor tem buraco.
2. Existe e nao esta em `pt/`?  Ai sim, falta traduzir.
3. Nao existe em lugar nenhum do fonte?  A frase esta cravada no codigo e
   o mod nao alcanca -- veja 14.5.

### 14.2 O erro mais perigoso: lote publicado que nao esta em `pt/`

O lote 32 foi ao ar na 0.31.0.  Em `pt/` havia so `chaves32.json` e
`trans32.json` -- **a planilha em INGLES**.  `pt.carregar()` nao le JSON,
entao o catalogo nao tinha aquelas 158 chaves.

O que isso faz: como a precedencia e *nossa > derivada > ingles*, sem a
nossa a **derivada de terceiros ganha de novo**.  O primeiro rebuild
devolveu 156 falas para o texto de R_Lopes/Night_Shadown.  Sem erro, sem
aviso, e com a contagem total identica (2951 falas nos dois casos) --
porque uma derivada entrou no lugar exato de cada uma que saiu.

**Regra dura, sem excecao:**

```bash
git diff lang/dialogue.lua
```

Antes de todo commit.  **Qualquer linha comecando com `-` e regressao.**
Um lote novo so pode ADICIONAR.  Se aparecer remocao, pare e descubra qual
lote sumiu do catalogo antes de seguir.

E a regra que evita a causa: **traducao mora em `pt/dialogo_NN.py`, ponto.**
JSON serve para planilha de trabalho (o ingles), nunca para o resultado.
Se voce gerou traducao em JSON, converta para `.py` e registre em
`pt/__init__.py` ANTES de buildar.

Se acontecer de novo, da para resgatar: o texto sobrevive dentro do
`lang/dialogue.lua` publicado.  `_resgatar_lote32.py` faz isso -- le
`git show HEAD:lang/dialogue.lua`, cruza com as chaves do lote e escreve o
`.py` que faltava.

### 14.3 A varredura parava no banco 0x5f

`varrer.py` roda `range(0x40, 0x60)`.  Os bancos 0x60 a 0x7f nunca foram
varridos, e tem coisa la:

| Bancos | Quanto | O que e |
|---|---|---|
| 60, 61 | **15** | falas de dialogo de verdade, **em ingles**: GINASIO de SAFFRON, SILPH CO., o laboratorio do ELM |
| 68-6b | 383 | **entradas da POKéDEX** -- categoria inteira, nunca tocada |
| 6d | 251 | **descricoes de golpes** |
| 6e | 159 | **descricoes de itens** |
| 64-66 | 401 | mensagens que o motor reimplementou em Lua: sao as `Strings.source` de 14.1, NAO se traduz por ponteiro |
| 70 | 58 | Game Boy Printer |

Para varrer os bancos altos:

```bash
python -c "import varrer as V, walk as W, json; usa=W.Rom(W.USA_PATH); d=json.load(open('dialogo.json',encoding='utf-8')); a=V.varrer(usa,range(0x60,0x80)); print(len({k:v for k,v in a.items() if k not in d}))"
```

**Cuidado ao gravar:** so os bancos 60 e 61 sao dialogo.  Gravar 64-70 no
`dialogo.json` enche o catalogo de chaves que o motor nunca consulta por
ponteiro, e o `progresso.py` passa a mentir para baixo para sempre.

POKéDEX, golpes e itens tem rota propria, e melhor:

```lua
mod.content.pokemon:patch("BULBASAUR", { dexEntry = { text = "..." } })
mod.content.moves:patch("TACKLE",      { description = "..." })
mod.content.items:patch("POTION",      { description = "..." })
```

`dexEntry` esta declarado no schema.  `description` **nao** esta declarado
em `moves`/`items`, mas o validador deixa o registro de topo extensivel
(`src/mods/Schemas.lua:179`) e quem desenha le `def.description`
(`PackMenu.lua:866`, `SummaryMenu.lua:604`).  Deve funcionar -- **teste com
um item so antes de escrever 600.**

### 14.4 As ferramentas mediam a pasta errada

O `progresso.py` conta as publicadas lendo
`scratchpad/versaodourada/lang/dialogue.lua` -- a pasta de **build local**,
nao o repositorio.  Na auditoria ela estava congelada na 0.25.0, entao o
relatorio dizia "2803 publicadas" quando o repo tinha 2951.  Um numero
errado que parece certo por varias versoes seguidas.

Do mesmo jeito: `build_mod.py` estava com `VERSION = "0.25.0"` com o repo
em 0.31.0, e `CHANGELOG_mod.md` / `README_mod.md` do scratchpad estavam
parados na 0.25.0 -- e **o build copia esses dois por cima** dos do pacote.
Publicar sem sincronizar teria apagado o historico das versoes 0.26 a 0.31.

**Antes de qualquer build, sincronize a base a partir do repositorio:**

```bash
cp "D:/pokemon gold tradução/versaodourada/CHANGELOG.md" CHANGELOG_mod.md
```

O mesmo para o `README.md`.  E confira a versao contra o `manifest.json` do
repo, nunca contra a memoria.

### 14.5 As 109 frases que o mod NAO alcanca

Ha texto de jogo cravado direto no codigo `gen2`, fora de `Strings()` e
fora de `Strings.source()`.  Ex.: `src/battle/gen2/Battle.lua:1213` monta
`self:emit({ kind = "message", text = "A critical hit!" })`, e
`src/core/gen2/ItemEffects.lua:78` guarda `"%s's\ncured of poison."` numa
tabela crua.

Nao ha registro que chegue nelas.  **Nao gaste lote nisso** -- e issue para
o upstream.  A lista sai de `_intraduzivel.py`.  As piores:

- `battle/gen2/Battle.lua` (14) -- "A critical hit!", "It's super
  effective!", "It hurt itself in its confusion!", "Got away safely!"
- `ui/gen2/Pokegear.lua` (15) -- radio e telefone
- `ui/gen2/BattleState.lua` (13) -- as mensagens de captura
- `battle/gen2/Effects.lua` (12) -- todo o clima
- `core/gen2/ItemEffects.lua` (11) -- usar POTION, ANTIDOTE, FULL HEAL

### 14.6 Armadilhas de ferramenta que custaram tempo

**Ler o `dialogue.lua` de volta sem `\f` e `\v` no mapa de unescape.**  O
arquivo escreve `\n`, `\f` e `\v`.  Um mapa que so trata `\n` transforma a
fala inteira numa linha so, e **80% das falas parecem estourar as 18
colunas** -- inclusive as suas, que estao certas.  Se uma medicao acusar
defeito em quase tudo, o defeito e da medicao.  **Rode sempre contra as
NOSSAS falas primeiro, como controle:** se elas tambem "falham", pare e
conserte a ferramenta.

**Heredoc do bash come a barra invertida.**  `python - <<'PY'` com um regex
`[^"\\]` chega no Python como `[^"\]` e explode com *unterminated character
set*.  Aconteceu duas vezes nesta auditoria.  Use a ferramenta Write para
criar o `.py` e depois rode `python arquivo.py`.  O mesmo vale para
`cat >> arquivo <<'FIM'` com texto longo: escreva o arquivo e concatene com
Python.

**Os rotulos de status continuam vazios de proposito, por ora.**
`lang/status_labels.lua` sai vazio porque nenhum lote define `STATUS`.  O
`main.lua` chama `mod.content.statuses:patch(id, ...)`; um `id` que nao
exista pode derrubar o carregamento do mod inteiro.  Os ids do Gold estao
em `src/battle/gen2/Battle.lua:2740` (`sleep`, `poison`, `toxic`,
`paralyze`, `burn`, `freeze`) -- **confirme-os e teste com UM antes de
mandar os cinco.**

### 14.7 Um erro que NAO era erro

A auditoria "corrigiu" o README de 2419/532 para 2261/690 -- e estava
errada.  Os numeros do README so voltaram a bater depois que o lote 32 foi
resgatado (2417/534).  A contagem estava certa; o CATALOGO e que estava
incompleto.

Licao: quando um numero do README nao bate com o que o build imprime, a
primeira hipotese e **catalogo incompleto**, nao README errado.  Confira o
catalogo antes de mexer no texto da pagina.

### 14.8 A ordem do que falta, medida em 14/08/2026

1. **128 strings `Strings.source` que sobraram** (das 212; a 0.32.0 pegou
   86).  Em ingles na tela agora.  A maior parte esta em
   `src/script/gen2/Specials.lua` (75): Concurso de Insetos, Day-Care,
   Apagador de Golpes, Avaliador de Nomes, a pergunta de apelido.
2. **15 falas de dialogo nos bancos 60/61** (14.3).  Em ingles na tela.
3. **POKéDEX (251) + golpes (251) + itens (~180)** -- categorias inteiras
   em ingles, com rota propria de registro.
4. **534 falas ainda derivadas** -- ja aparecem em portugues; e o que
   segura a atribuicao virar agradecimento.
5. **Rotulos de status** (14.6), depois de testar com um.
6. **As 109 cravadas** (14.5) -- issue no upstream, nao lote.
